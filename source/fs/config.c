...
EFI_STATUS config_load(config_entry_t** entries, UINTN* count, EFI_HANDLE image)
{
	...
	// Set return parameters to default
	*entries = NULL;
	*count = 0;
	...
	status = uefi_call_wrapper(volume->Open, 5,
		volume, &config_file, CONFIG_FILE_PATH,
	...
	status = uefi_call_wrapper(BS->AllocatePool, 3,
		PoolAllocationType, buffer_size + 1, (VOID**)&buffer
	);
	if(EFI_ERROR(status))
	{
	...
	// Read file content
	status = uefi_call_wrapper(config_file->Read, 3,
		config_file, &buffer_size, (VOID*)buffer
	);
	if(EFI_ERROR(status))
	{
...
EFI_STATUS config_parse(CHAR8* buffer, UINTN size, config_entry_t** entries, UINTN* count)
{
	...
	// Allocate entry list
		status = uefi_call_wrapper(BS->AllocatePool, 3, 
		PoolAllocationType, MAX_ENTRIES * sizeof(config_entry_t),
		(VOID**)&config_entries
	);
	if(EFI_ERROR(status))
	{
		...
		if(*lstart == '[' && *(lend - 1) == ']')
		{
			...
			// Set entry name
			{
				...
				for (UINTN i = 0; i < length; i ++)
					current_entry->name[i] = (CHAR16)(lstart[i + 1]);
